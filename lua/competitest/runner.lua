...
function TCRunner:new(bufnr)
  ...
  if this.rc == nil then
    utils.notify("TCRunner:new: run command for filetype '" ..
      filetype .. "' isn't configured properly.\nCannot proceed.")
    return nil
  end
...
function TCRunner:execute_testcase(tcindex, exec, args, dir, callback)
  ...
  if not process.handle then
    utils.notify("TCRunner:execute_testcase: failed to spawn process using '" ..
      process.exec .. "' (" .. process.pid .. ").")
    tc.status = "FAILED"
    tc.hlgroup = "CompetiTestWarning"
    tc.time = -1
    self:update_ui(true)
